"""Integration utilities for legacy image deduplication pipeline.

This module provides lightweight wrappers to reuse artefacts produced by
the historical image dedup workflow (embedding extraction, clustering and
SemDeDup pruning).  The goal is to expose the minimal pieces required by the
new orchestrated pipeline so we can progressively migrate functionality
without rewriting everything from scratch.

Current responsibilities:

* Load pre-computed embeddings (`image_embeddings.npy`) and their associated
  path index (`image_paths.npy`), supporting raw memmap fallbacks when they
  were exported via numpy memmap instead of the standard `.npy` header.
* Align embeddings with a manifest list of image paths, returning the
  per-image vectors alongside a list of missing images.
* Provide type-light data structures that can be consumed by the modern
  `pipeline_api` code.

Future work will extend this module with clustering helpers and wrappers
around the SemDeDup scoring logic so that the orchestrated pipeline can call
into the legacy implementation for the heavy lifting.
"""
...
def _normalise_path_text(text: str) -> List[str]:
    """Generate candidate keys for matching file paths across platforms."""
...
def _load_numpy_array(path: Path, expected_dim: Optional[int] = None) -> np.ndarray:
    """Load numpy data with best-effort fallbacks for raw memmap files."""
...
def load_legacy_embeddings(
    ...
    """Load legacy embeddings and align them with the provided manifest.

    Parameters
    ----------
    embedding_file:
        Path to the `.npy` (or raw memmap) file containing float32 vectors.
    index_file:
        Path to the `.npy` file storing the original image paths (usually
        produced alongside `image_embeddings.npy`).
    manifest_paths:
        Iterable of Paths that should be looked up in the legacy artefacts.

    Returns
    -------
    LegacyEmbeddings
        Dataclass containing the stacked vectors for the images that were
        found, the corresponding resolved paths, and a list of missing entries
        for images not present in the legacy index.
    """
    ...
    # Build lookup table from normalised path string to embedding row index.
    lookup: Dict[str, int] = {}
...
def save_embeddings_snapshot(
    ...
    """Persist embeddings + paths in the legacy on-disk format."""
